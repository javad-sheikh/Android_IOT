...
// Place your code here
 delay_ms(50);
     state=PORTC&(1<<PORTC5);
               if(state==0)
               {
                  PORTC|=(1<<PORTC5);   //TURN ON LAMP
                       PORTC&=~(1<<PORTC4);  //LED FOR NIGHT SIGN 
               }
               else
               {
                     PORTC&=~(1<<PORTC5);   //TURN OFF LAMP
                    PORTC|=(1<<PORTC4);  //LED FOR NIGHT SIGN 
               }
               GICR|=(0<<INT1) | (1<<INT0);
}
...
interrupt [USART_RXC] void usart_rx_isr(void)
{
   ...
   if ((status & (FRAMING_ERROR | PARITY_ERROR | DATA_OVERRUN))==0)
   {
      ...
      {             
       ...
       {   
        /*  if (counter_str==5 & data=='N')
          {
          
          } */
          str[counter_str]=data;
          counter_str++;
       } 
      }
...
PORTC=(0<<PORTC6) | (0<<PORTC5) | (1<<PORTC4) | (0<<PORTC3) | (0<<PORTC2) | (0<<PORTC1) | (0<<PORTC0);
